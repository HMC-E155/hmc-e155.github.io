...
It is useful to simulate them before putting them on hardware. It is standard industrial practice to verify all Verilog before shipping it as a product, often through simulation, so this course requires simulation coverage of all Verilog features. In addition, simulation can give you visibility into design details that hardware indicators cannot, so it is a useful practice (especially for lab 3).
...
The inputs start floating (HiZ), so initialize them with `force <Signal Name> <Signal Value>` in the Transcript pane. Signal value must have its bit width specified.
...
### Designing a Good Testbench {#designing-a-good-testbench}
...
Be careful of the timing of your test signals. To show that an enable signal is needed for a clock to count, the enable must be asserted after the reset, and it must be asserted for longer than a clock cycle (otherwise it will have no effect on the count).  Similarly, if you are showing reset mid-simulation, it should be on a different cycle than the max count overflow to prove that is indeed the reset signal causing count to go to zero. 

In addition to timing the stimulus to demonstrate features, you also need to capture outputs that show the feature at work.  Be sure to show the clock and the count so readers can observe the count changing as reset and enable are toggled.

**Top-level modules — only test what the module tests didn't cover.**
Once each submodule is verified on its own, re-testing that logic through the top level adds nothing. A top-level testbench should target what is genuinely new at that level:

- Are the submodules wired together correctly?
- Does anything instantiated only at the top level work — for example, does the `HSOSC` produce a clock?
- Does logic written directly in the top level (such as an `assign` statement) behave correctly?
...
Testbenches for top modules are often quite simple (other than verifying logic that is not contained in a module).  If the clock is running and the outputs toggle properly, then the HSOSC and submodule connections are probably right.  Make sure the HSOSC output clock signal is included in your simulation screenshot to show that you meet these criteria.

::: {.callout-important title="Working on Lab 1? Return to the lab now"}
That is everything you need to plan your Lab 1 testbenches.
Go back to [Lab 1 → Automatic Testing with a Testbench](/lab/lab1/#automatic-testing-with-a-testbench) and write yours.
