...
- [ ] Development board is fully assembled (e.g., all parts soldered) and FPGA is programmed with a Verilog design.
...
- [ ] Resistors are in series with FPGA pins so that they correctly limit current, not in shut to ground.

#### Excellence

- [ ] All digits are equally bright, regardless of the number of segments illuminated.
- [ ] LED segments are fully off when disabled (no residual glow, check by shading it with you hand).
- [ ] LED blink rate is close to 2.4Hz. A 1.2Hz or 4.8Hz blink rate fails this spec. This requires the 7 segment display to be powered off of 3.3V, not 5V.

### Verilog Structure and Style Specifications

#### Proficiency

- [ ] assign used to implement switch-to-LED logic in the top level module. (Note that it is standard practice to avoid putting 1 liner logic in a module; doing so would add more lines of code than it saves!)
...
- [ ] Counter logic contained in a module with inputs: `reset_b`, `enable` and `clk`, and output: `count`.
- [ ] Counter includes reset, enable and settable (possibly hardcoded) max count features. The enable feature causes the counter to only increment when an enable pin is asserted. The max count feature causes the counter to set its count back to zero when the count reaches the max value.
- [ ] Top level consists only of instantiated modules, switch-to-LED and count-to-LED logic.
...
- [ ] All testbenches in separate modules and files, mirroring structure of Verilog files.
...
- [ ] No errors in LED frequency calculations (not off by a factor of 2, uses appropriate clock rates, etc.)
