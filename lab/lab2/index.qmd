...
One way to interrogate a matrix keypad is to assert signals on each row one at a time, then observe the values of the columns.  The polling module you are building is designed to carry out just such an interrogation. 

The specifications are prescriptive about the structure of this polling module: it should consist of a counter and some output logic. That means that the polling module is a kind of non-canonical state machine: the state register and state update logic are contained in the counter, and the output logic you will implement converts from the state (the count) to the four outputs.
